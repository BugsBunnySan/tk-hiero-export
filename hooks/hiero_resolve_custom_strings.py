...
class HieroResolveCustomStrings(Hook):
    ...
    def _formatSequenceString(self, keyword, task, sg_shot):
        sg_sequence = None
        if sg_shot['sg_sequence']['id'] not in HieroResolveCustomString._sg_lookup_cache['Sequence']:
            sg_sequence = find_one_entity_by_id('Sequence', sg_shot['sg_sequence']['id'], fields=['code'])
            if not sg_sequence:
                return ''
            HieroResolveCustomString._sg_lookup_cache['Sequence'][sg_shot['sg_sequence']['id']] = sg_sequence
        else:
            sg_sequence = HieroResolveCustomString._sg_lookup_cache['Sequence'][sg_shot['sg_sequence']['id']]
        return sg_sequence['code']
        
    def _formatSceneString(self, keyword, task, sg_shot):
        sg_scene = None
        if sg_shot['sg_scene']['id'] not in HieroResolveCustomString._sg_lookup_cache['Scene']:
            sg_scene = find_one_entity_by_id('Scene', sg_shot['sg_scene']['id'], fields=['code'])
            if not sg_scene:
                return ''
            HieroResolveCustomString._sg_lookup_cache['Scene'][sg_shot['sg_scene']['id']] = sg_scene
        else:
            sg_scene = HieroResolveCustomString._sg_lookup_cache['Scene'][sg_shot['sg_scene']['id']]
    ...
    def execute(self, task, keyword, **kwargs):
        ...
        if lookup_key in HieroResolveCustomString._sg_lookup_cache['Shot']:
            sg_shot = HieroResolveCustomString._sg_lookup_cache['Shot'][lookup_key]
        else:
            sg_shot = find_one_entity_by_code("Shot", task._item.name(), self.parent.app.get_setting("custom_template_fields"))
            HieroResolveCustomString._sg_lookup_cache['Shot'][lookup_key] = sg_shot
        if not sg_shot:
            return ''            
